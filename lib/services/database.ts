...
export const getDatabase = (): any => {
  if (isNative) {
    ...
    return {
       ...
       getAllAsync: async (query: string, ...rawParams: any[]) => {
    const params = normalizeParams(rawParams);

    const tableName = extractTableName(query);
    let data = inMemoryDB[tableName] || [];

    if (query.includes('LEFT JOIN')) data = handleJoinQuery(query, params);
    else if (query.includes('WHERE')) data = filterData(data, query, params);

    if (query.includes('COUNT(*)')) return [{ count: data.length }];

    if (query.includes('ORDER BY')) data = sortData(data, query);
    ...
    return data;
      ...
      getFirstAsync: async (query: string, ...rawParams: any[]) => {
    const params = normalizeParams(rawParams);
    const results = await getDatabase().getAllAsync(query, params);
    return results.length > 0 ? results[0] : null;
  },

  runAsync: async (query: string, ...rawParams: any[]) => {
    const params = normalizeParams(rawParams);
    const tableName = extractTableName(query);
    ...
    if (query.trim().toUpperCase().startsWith('INSERT')) {
      ...
      if (inMemoryDB[tableName]) {
        inMemoryDB[tableName] = inMemoryDB[tableName].map((record: any) => {
          if (matchesWhere(record, query, params)) return { ...record, ...updates };
          return record;
        });
      }
...
// function filterData(data: any[], query: string, params: any[] = []) {
//   const whereMatch = query.match(/where\s+([a-z0-9_]+)\s*=\s*\?/i);
//   if (!whereMatch) return data;

//   const col = whereMatch[1];
//   const val = params[0]; // for SELECT ... WHERE col = ? (first param)
//   return data.filter(r => String(r[col]) === String(val));
// }
...
//   }
//     if (query.includes('WHERE code = ?')) {
//     return record.code === params[params.length - 1];
//   }
...
//   return true;
// }
function matchesWhere(record: any, query: string, params: any[] = []) {
  const whereMatch = query.match(/where\s+([a-z0-9_]+)\s*=\s*\?/i);
  if (!whereMatch) return true; // no WHERE => affects all (expected)

  const col = whereMatch[1];
  const val = params[params.length - 1]; // for UPDATE ... WHERE col = ? (last param)
...
//   return updates;
// }
 function parseUpdate(query: string, params: any[]) {
  // Extract "SET a=?, b=?, c=?"
  const setMatch = query.match(/set\s+(.+?)\s+where/i);
  if (!setMatch) return {};

  const setPart = setMatch[1]; // "a = ?, b = ?, c = ?"
  const cols = setPart.split(',').map(s => s.split('=')[0].trim());

  // values for SET are params except the last one (WHERE)
  const setValues = params.slice(0, cols.length);

  const obj: any = {};
  cols.forEach((c, i) => (obj[c] = setValues[i]));
  return obj;
}
...
drop table customers;
  ...
  PRIMARY KEY ( code)
    ...
    CREATE TABLE IF NOT EXISTS schedules (
      ...
      FOREIGN KEY (routecode) REFERENCES routes(code)
    );
    ...
    CREATE TABLE IF NOT EXISTS van_loads (
      ...
      created_at TEXT NOT NULL,
      FOREIGN KEY (van_id) REFERENCES vans(id),
      FOREIGN KEY (warehousecode) REFERENCES warehouses(code)
    );
    ...
    CREATE INDEX IF NOT EXISTS idx_vans_warehouse ON vans(warehousecode);
    CREATE INDEX IF NOT EXISTS idx_vans_user ON vans(usercode);
    CREATE INDEX IF NOT EXISTS idx_vans_route ON vans(routecode);
    CREATE INDEX IF NOT EXISTS idx_customers_category ON customers(category_id);
    CREATE INDEX IF NOT EXISTS idx_customers_route ON customers(routecode);
    ...
    CREATE INDEX IF NOT EXISTS idx_schedules_user ON schedules(usercode);
    CREATE INDEX IF NOT EXISTS idx_schedules_route ON schedules(routecode);
  `);
...
const seedInitialDataNative = async () => {
  ...
  // await db.runAsync(
  //   `INSERT INTO users (id, usercode, user_name, password_hash, role, default_van_id, is_active, created_at, updated_at) VALUES
  ...
  // await db.runAsync(
  //   `INSERT INTO vans (id, code, vehiclenumber, warehousecode, usercode, routecode, is_active, created_at, updated_at) VALUES
  //     (?, 'VAN001', 'ABC-1234', ?, ?, ?, 1, ?, ?),
  //     (?, 'VAN002', 'DEF-5678', ?, ?, ?, 1, ?, ?)`,
  //   [
  ...
  for (const c of customers) {
    ...
    customerIds.push(id);
    await db.runAsync(
      `INSERT INTO customers (id, code, name, category_id, routecode, phone, email, address, latitude, longitude, credit_limit, outstanding_balance, is_active, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, NULL, ?, ?, ?, ?, 0, 1, ?, ?)`,
  ...
  await db.runAsync(
    `INSERT INTO schedules (id, usercode, routecode, schedule_date, status, notes, created_at, updated_at) VALUES
...
const seedInitialDataWeb = () => {
  ...
  // const userIds = [generateUUID(), generateUUID(), generateUUID()];
  // inMemoryDB.users = [
  //   { id: userIds[0], usercode: 'manager', user_name: 'Sales Manager', password_hash: 'manager', role: 'SALES_MANAGER', default_van_id: null, is_active: true, created_at: now, updated_at: now },
  //   { id: userIds[1], usercode: 'sm1', user_name: 'Mike Johnson', password_hash: 'sm1', role: 'SALESMAN', default_van_id: null, is_active: true, created_at: now, updated_at: now },
  //   { id: userIds[2], usercode: 'sm2', user_name: 'Sarah Williams', password_hash: 'sm2', role: 'SALESMAN', default_van_id: null, is_active: true, created_at: now, updated_at: now },
  // ];

  // const vanIds = [generateUUID(), generateUUID()];
  // inMemoryDB.vans = [
  //   { id: vanIds[0], code: 'VAN001', vehiclenumber: 'ABC-1234', warehousecode: warehouseIds[0], usercode: userIds[1], routecode: routeIds[0], is_active: true, created_at: now, updated_at: now },
  //   { id: vanIds[1], code: 'VAN002', vehiclenumber: 'DEF-5678', warehousecode: warehouseIds[0], usercode: userIds[2], routecode: routeIds[1], is_active: true, created_at: now, updated_at: now },
  // ];
  ...
  inMemoryDB.schedules = [
    { id: schedule1Id, usercode: userIds[1], routecode: routeIds[0], schedule_date: today, status: 'planned', notes: 'Downtown route schedule', created_at: now, updated_at: now },
    { id: schedule2Id, usercode: userIds[2], routecode: routeIds[1], schedule_date: today, status: 'planned', notes: 'North district schedule', created_at: now, updated_at: now },
  ];
